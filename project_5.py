...
# Professional Dark Theme with Neutral Corporate Blue (#00d4ff)
st.markdown("""
    <style>
...
def fetch_corporate_data(ticker):
    ...
    try:
        ...
        # If API returns empty data, trigger the fallback
        if is_stmt.empty or 'Total Revenue' not in is_stmt.index: 
            raise ValueError("Data Restricted")
        ...
        ratios = {
            ...
            "ROIC": info.get("returnOnAssets", 0.05) * 2.2, # Proxy for ROIC
        ...
        # --- UNIVERSAL FALLBACK (Normalized Industry Averages) ---
        # This allows you to show the dashboard logic even if API fails
        st.sidebar.warning(f"⚠️ Live Data for {ticker} Restricted. Using Sector Benchmarks.")
        
        years = [2021, 2022, 2023, 2024]
        leverage_df = pd.DataFrame({
            "Revenue": [100, 115, 135, 160], # Indexed to 100
            "Op_Income": [15, 18, 22, 28]
        }, index=years)
        
        fcf_per_share = pd.Series({2021: 1.0, 2022: 1.2, 2023: 1.5, 2024: 1.9})
        ratios = {"OCF_Margin": 0.15, "ROIC": 0.12, "CapEx_Sales": 0.07}
        price, fcf_now, shares, name, debt, cash = 100.0, 1e9, 1e9, f"{ticker} (Benchmark)", 1e9, 2e9
...
ticker_input = st.sidebar.text_input("Enter Global Ticker", value="AAPL").upper()
st.sidebar.markdown("---")
growth_input = st.sidebar.slider("Projected FCF Growth (%)", 0, 50, 15) / 100
wacc_input = st.sidebar.slider("Discount Rate (WACC %)", 5, 15, 10) / 100
sim_input = st.sidebar.select_slider("Simulations", options=[500, 1000], value=500)

# --- 4. DASHBOARD EXECUTION ---
data = fetch_corporate_data(ticker_input)

st.title(f"🏢 Corporate Strategy Terminal: {data['name']}")
st.caption("Developed by Megha R Ajit | ESCP Business School | Strategy & Financial Analysis")
...
c4.metric("Current Price", f"${data['price']:.2f}")

# SECTION 2: OPERATING LEVERAGE
st.markdown("---")
st.markdown("### 📈 Operating Leverage Analysis")
st.write("Visualizing the relationship between top-line Revenue and Operating Income scalability.")

fig_lev = go.Figure()
fig_lev.add_trace(go.Scatter(x=data['leverage'].index, y=data['leverage']['Revenue'], name="Total Revenue", line=dict(color='#00d4ff', width=4)))
fig_lev.add_trace(go.Bar(x=data['leverage'].index, y=data['leverage']['Op_Income'], name="Operating Income", marker_color='#2ECC71', opacity=0.6))
